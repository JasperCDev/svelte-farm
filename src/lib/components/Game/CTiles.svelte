...
<script lang="ts">
    ...
    let tilesPiecesToRender = $derived(
        derive(() => {
            let t = farmLand.tilePieces.map((tp) => {
                return {
                    tp,
...
{#each tilesPiecesToRender as tilePiece}
    <div
        class="tile-piece"
        style="
      top: {tilePiece.tp.point.row * farmLand.tileSize - farmLand.tileSize * 1.5}px;
      left: {tilePiece.tp.point.col * farmLand.tileSize - farmLand.tileSize * 1.5}px;
